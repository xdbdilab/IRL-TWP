...
parser.add_argument('--experiment-id', default="gail_experiment_0726-4", metavar='G',
                    help='the id of this experiment')
parser.add_argument('--traj-path', default="/27T/TE_TWP/traj_100_train/", metavar='G',
...
def main_loop():
    ...
    for i_iter in range(args.max_iter_num):
        ...
        if i_iter % args.log_interval == 0:
            ...
            if len(error_array) > args.error_memo_length:
                ...
                if log['avg_mse']<2.4:
                    ...
                    filename = '/27T/TE_TWP/GAIL/gail/assets/learned_models/' + args.experiment_id + \
        ...
        if args.save_model_interval > 0 and (i_iter+1) % args.save_model_interval == 0:
            to_device(torch.device('cpu'), policy_net, value_net, discrim_net)
            filename = '/27T/resource-simulator/GAIL/gail/assets/learned_models/' + args.experiment_id + \
                       '_{}.p'.format(log['avg_mse'], i_iter + 1)
            os.makedirs(os.path.dirname(filename), exist_ok=True)
            with open(filename, 'wb') as f:
                pickle.dump((policy_net, value_net, discrim_net), f)
